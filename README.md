...
## 🧠 训练你自己的智能模型 (N-gram Training)

为了让输入法更懂你的输入习惯，你可以使用自己的语料进行训练：

1. **准备语料**：将你的文章、聊天记录或常用语存为 `my_text.txt` (UTF-8 编码)。
2. **执行训练**：
   ```bash
   rust-ime --train my_text.txt
   ```
3. **生效**：训练完成后，重启服务 `rust-ime --restart`。
4. **原理**：程序会学习词语之间的二元关联（Bigram），并保存在 `ngram.json` 中。此过程是**增量**的，你可以多次训练不同的文件。
