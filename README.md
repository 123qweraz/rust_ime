...
*   **笔画特征分析 (统计数据)**：
    *   **重码率极低**：在全量 2 万字字典中，超过 **95%** 的编码是唯一的。即使在重码情况下，绝大多数编码也仅对应 2 个汉字，基本实现“所见即所得”的盲打体验。
    *   **笔画数分布**：6 笔及以下的汉字仅占总量的约 **6%** (常用字中约占 10%)。这意味着绝大多数汉字通过 SBSRF 的 4 键编码即可实现精准区分，几乎不需要选词。
